...
## 🔴 Дефект 2 — `save()` выполняет UPDATE вместо INSERT для новых счетов
...
## 🟡 Дефект 3 — Entity не отображает 4 обязательных столбца схемы
...
## 🟡 Дефект 4 — Отсутствует `spring-boot-starter-jdbc` в `vault-transfers`
